...
def calculate_average_respiratory_admissions(copd_hits):
    ...
    for station_id, categories in admissions_data.items():
        average_admissions[station_id] = {
            'average_total': sum(categories['total']) / len(categories['total']),
            'average_males': sum(categories['males']) / len(categories['males']),
            'average_females': sum(categories['females']) / len(categories['females'])
        }
    ...
    for hit in air_quality_hits:
        ...
        if site_id in average_respiratory_admissions:
            ...
            merged_data.append({
                ...
                "Average total admissions": averages['average_total'],
                "Average male admissions": averages['average_males'],
                "Average female admissions": averages['average_females']
            })
